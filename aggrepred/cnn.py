...
class Masked1dConvolution(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sequence_lengths: torch.LongTensor) -> torch.Tensor:
        """
        Forward pass.
        ...
        mask = generate_mask(o, sequence_lengths)

        print("size of output o after cnn",o.size())
...
def generate_mask(input_tensor: torch.Tensor, sequence_lengths: torch.LongTensor) -> torch.Tensor:
    """
    Generate a mask for masked 1d convolution. values beyond the sequence length are zeroed out.

    :param input_tensor: an input tensor for convolution (bsz x features x seqlen)
    :param sequence_lengths: x of sequences (bsz,)
    :return: (bsz x features x seqlen)
    """
    ...
    mask = torch.arange(max_seqlen).expand(batch_size, channels, max_seqlen) < sequence_lengths.view(-1, 1, 1)
    return mask
